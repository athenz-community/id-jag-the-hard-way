...
3. **`components/mcp-gateway/`** — Node.js/TypeScript authenticated MCP front door for the MCP Hub demo. It performs Keycloak login, keeps the ID token in a server-side session, resolves routes and exact `tools/call.params.name` access scopes through the MCP Hub API, exchanges ID token → ID-JAG → Athenz access token per session and normalized scope, and forwards `/mcp/{id}` through Core MCP Proxy. It does not replace or change the tutorial's `components/ai_client_gateway/` yet.
