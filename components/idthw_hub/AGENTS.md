...
- Creating a Hub-managed MCP server uses that same server-side certificate to idempotently provision the shared access path in the existing `mcp-hub.mcps.<project>` domain: the `accessor` role with the signed-in user, the `accessor-jag-exchanger` role with `mcp-hub.mcp-gateway`, and the `zts.jag_exchange` policy from the exchanger role to the accessor role. The Hub verifies that the selected service exists, generates a dedicated RSA service key, registers its public key under the deterministic per-server key ID `idthw-hub-<mcp-key>` (truncated with a stable hash when necessary), and enables the `zts_instance_launch_provider` template for that service; it never creates the project domain. Multiple MCP servers may share one Athenz service account because their public-key IDs are distinct. Updates preserve legacy key IDs already stored in Runtime Proxy configuration.
