...
- Creating a Hub-managed MCP server uses that same server-side certificate to idempotently provision the shared access path in the existing `mcp-hub.mcps.<project>` domain: the `accessor` role with the signed-in user, the `accessor-jag-exchanger` role with `mcp-hub.mcp-gateway`, and the `zts.jag_exchange` policy from the exchanger role to the accessor role. The Hub verifies that the selected service exists, generates a dedicated RSA service key, registers its public key as `idthw-hub-generated`, and enables the `zts_instance_launch_provider` template for that service; it never creates the project domain. A service account can back only one generated-identity MCP server because that key ID is fixed.
