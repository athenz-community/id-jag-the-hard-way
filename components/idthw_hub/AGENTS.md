...
- Permission setup must enumerate tools from the public live MCP `tools/list` result, not from the preset. The YAML maps execution requirements onto those tool names; each tool repeats any shared MCP-access requirements so the UI has no separate server-access section. Tools without a preset remain visible with a warning, and request dialogs are informational until a real permission-request workflow exists.
- `/api/mcp-servers` derives `toolScopes` from each configured tool's `<signed_in_user>` roles. MCP Gateway uses the exact `tools/call.params.name` mapping and fails closed for an unmapped tool whenever a server has a configured map. The Kubernetes `access-scope` annotation remains only a legacy route-level fallback for servers without `toolScopes` and for protected non-tool methods.
