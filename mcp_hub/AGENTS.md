...
- Permission setup must enumerate tools from the public live MCP `tools/list` result, not from the preset. The YAML maps execution requirements onto those tool names; tools without a preset remain visible with a warning, and request dialogs are informational until a real permission-request workflow exists.
