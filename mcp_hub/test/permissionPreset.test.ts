...
test("loads the checked-in pure YAML permission settings", async () => {
  ...
  assert.equal(preset.groups[1].label, "Tool: get_k8s_docs")
  assert.equal(preset.groups[2].label, "Tool: post_k8s_doc")
  assert.equal(preset.groups[3].label, "Tool: delete_k8s_doc")
  assert.equal(preset.groups[2].requirements[0].role, "api:role.docs-poster")
  assert.equal(preset.groups[3].requirements[0].role, "api:role.docs-deleter")
})
...
test("resolves the exact signed-in-user placeholder and keeps literal principals", () => {
  ...
  assert.equal(preset.groups[0].requirements[0].member, "human.idjag-learner")
  assert.equal(preset.groups[0].requirements[1].member, "mcp-hub.mcp-gateway")
  assert.equal(preset.groups[1].label, "Tool: get_k8s_docs")
  assert.equal(preset.groups[1].toolName, "get_k8s_docs")
})
...
test("returns no preset for a server that is intentionally not configured", () => {
  ...
  servers.broken = { serverRequirements: [{ member: "<unknown>", role: "api:role.reader" }] }
...
test("rejects unknown and partial member placeholders", () => {
  for (const member of ["<current_user>", "human.<signed_in_user>"]) {
    ...
    configured.servers["k8s-docs-server"].serverRequirements[0].member = member
...
test("rejects malformed principals, roles, and unknown fields", () => {
  ...
  invalidPrincipal.servers["k8s-docs-server"].serverRequirements[1].member = "missing-domain"
  assert.throws(
  ...
  invalidRole.servers["k8s-docs-server"].serverRequirements[1].role = "api:role."
  assert.throws(
