...
The Authorization Server does not natively know how to handle tokens generated by a specific IdP. We need to install a plugin that teaches it how to process these tokens. The plugin files are located in the `components/keycloak_token_exchange_provider` directory. Let's apply a patch to our Athenz server to mount and use this plugin:
